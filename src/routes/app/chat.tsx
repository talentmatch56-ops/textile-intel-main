...
import { Send, Sparkles, MessageSquare, Plus, Loader2, Bot, User, Mic, MicOff, Volume2, VolumeX } from "lucide-react";
...
function getAIResponse(
  ...
  // 1. Contextual follow-up checks based on the last assistant message
  if (lastAssistantContent) {
    const lastLower = lastAssistantContent.toLowerCase();

    // Context: "Would you like a detailed comparison or help drafting an RFQ?"
    ...
    // Context: "Would you like me to draft a Request for Quote (RFQ) email template for Arvind Mills?"
    ...
    // Context: "Want a full risk report for a specific country or supplier?"
    if (lastLower.includes("want a full risk report for a specific country or supplier?")) {
      if (lower === "yes" || lower === "yes please" || lower === "sure" || lower === "ok" || lower.includes("bangladesh") || lower.includes("india")) {
    ...
    // Context: "Would you like to check compliance for one of these suppliers?"
    ...
    // Context: "Would you like to draft an inquiry to Arvind Mills for their Tier-2 certificate verification or run a risk report?"
    if (lastLower.includes("draft an inquiry to arvind mills for their tier-2 certificate verification")) {
      if (lower === "yes" || lower === "yes please" || lower === "sure" || lower === "ok" || lower.includes("inquiry") || lower.includes("draft") || lower.includes("email")) {
  ...
  // 2. Exact Company lookups by name
  for (const comp of companies) {
    const compNameLower = comp.name.toLowerCase();
    if (lower.includes(compNameLower) || (comp.slug && lower.includes(comp.slug.replace("-", " ")))) {
      return `Here is the current intelligence profile for **${comp.name}**:\n\n` +
  ...
  // 3. Dynamic aggregations & statistics
  if (lower.includes("highest trust") || lower.includes("best trust") || lower.includes("top trust") || lower.includes("most trusted")) {
    ...
    const top = sorted[0];
    if (top) {
      return `The supplier with the highest AI Trust Score in our database is **${top.name}** with a score of **${top.ai_trust_score}/100** (Location: ${top.city ?? "Primary Hub"}, ${getCountryName(top.country_code)}).\n\nOther high trust suppliers include:\n` +
        sorted.slice(1, 4).map(c => `- **${c.name}**: Trust ${c.ai_trust_score}/100`).join("\n") +
        `\n\nWould you like me to compare these top trusted suppliers?`;
    }
  }

  if (lower.includes("how many companies") || lower.includes("total companies") || lower.includes("how many suppliers")) {
    ...
    return `We are currently tracking **${total} companies** in the GMIntel system:\n` +
      `- **Manufacturers:** ${manufacturers}\n` +
      `- **Suppliers / Traders:** ${suppliers}\n` +
  ...
  if (lower.includes("gots") || lower.includes("organic") || lower.includes("sustainab")) {
    const gotsCertified = companies.filter(c => 
      c.name.toLowerCase().includes("arvind") || 
      c.name.toLowerCase().includes("vardhman") || 
      c.name.toLowerCase().includes("envoy") ||
      (c.business_type ?? "").toLowerCase().includes("organic")
    );
    return `Here are the key GOTS-certified/organic manufacturers in our registry:\n\n` +
      gotsCertified.slice(0, 4).map(c => `- **${c.name}** (${getCountryName(c.country_code)}) — Trust: ${c.ai_trust_score}/100 | Specializes in certified organic fiber.`).join("\n") +
  ...
  if (lower.includes("india") || lower.includes("indian")) {
    ...
    if (indians.length > 0) {
      return `We track **${indians.length} suppliers in India**:\n\n` +
  ...
  if (lower.includes("bangladesh") || lower.includes("bangladeshi")) {
    ...
    if (bd.length > 0) {
      return `We track **${bd.length} suppliers in Bangladesh**:\n\n` +
  ...
  if (lower.includes("turkey") || lower.includes("turkish")) {
    ...
    if (tr.length > 0) {
      return `We track **${tr.length} suppliers in Turkey**:\n\n` +
  ...
  // 4. Common Conversational phrases (greetings, thanks, negations)
  if (lower === "hello" || lower === "hi" || lower === "hey" || lower === "greetings") {
    return "Hello! I am your GMIntel AI Copilot. I can help you search verified suppliers, fetch live fabric prices, or compile risk alerts. What are you sourcing today?";
  }
  if (lower === "thanks" || lower === "thank you" || lower === "great" || lower === "awesome" || lower === "thanks!") {
    return "You're very welcome! Let me know if you need help generating a PDF supplier report or analyzing more price index points.";
  }
  if (lower === "no" || lower === "no thanks" || lower === "clear" || lower === "stop" || lower === "nope") {
    return "Understood. Let me know when you are ready to start a new sourcing run.";
  }

  // 5. Keyword matches fallback
...
function renderMarkdown(text: string) {
  // If text contains ```text, we can extract the codeblock
  if (text.includes("```text")) {
    const parts = text.split("```text");
    return parts.map((part, i) => {
      if (i % 2 === 1) {
        // This is inside the codeblock
        const codeContent = part.split("```")[0];
        return (
          <pre key={i} className="my-2 p-3 bg-muted/60 border border-border rounded font-mono text-xs overflow-x-auto whitespace-pre text-foreground">
            {codeContent.trim()}
          </pre>
        );
      } else {
        // Outside the codeblock, render as normal lines
        return renderNormalLines(part);
      }
    });
  }
...
function renderNormalLines(text: string) {
  const lines = text.split("\n");
  return lines.map((line, i) => {
    const isBullet = /^[-•]\s/.test(line);
    const content = line.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>");
    if (isBullet) {
      return <li key={i} className="ml-2 list-disc" dangerouslySetInnerHTML={{ __html: content.replace(/^[-•]\s/, "") }} />;
    }
...
function MessageBubble({ msg, onSpeak, playingVoiceId }: { msg: Message; onSpeak: (id: string, text: string) => void; playingVoiceId: string | null }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      <div className={`max-w-[80%] ${isUser ? "items-end" : "items-start"} flex flex-col gap-1`}>
        <div className={`rounded-lg px-4 py-3 text-sm leading-relaxed relative group ${
          isUser ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground"
        }`}>
          {isUser ? (
            <span className="whitespace-pre-wrap">{msg.content}</span>
          ) : (
            <div className="prose-chat space-y-1 pr-4">
              {hasListItems ? (
              ...
              {msg.isStreaming && (
                <span className="inline-block h-3.5 w-1.5 ml-0.5 bg-primary animate-pulse" />
              )}
            </div>
          )}

          {/* Voice Speech Trigger Button (only for AI assistant responses) */}
          {!isUser && !msg.isStreaming && (
            <button
              ...
              className={cn(
                ...
                isPlaying && "opacity-100 text-primary bg-primary/10"
              )}
...
function Page() {
  ...
  // Fetch verified suppliers list from Supabase dynamically for the AI's usage
  const { data } = useQuery({
    queryKey: ["dashboard"],
    queryFn: async () => {
      ...
      return { companies: companies.data ?? [], countries: countries.data ?? [], news: news.data ?? [] };
    },
  });

  const dbCompanies = data?.companies ?? [];
  const companies = useMemo(() => {
    const list: any[] = [...dbCompanies];
  ...
  }, [messages, activeThread, thinking]);
  ...
  const speakText = (msgId: string, text: string) => {
    if (typeof window !== "undefined") {
      ...
      window.speechSynthesis.cancel();
      const cleanText = text.replace(/\*\*/g, ""); // Strip stars
  ...
  // Real-time word streaming simulation
  const streamAIResponse = (responseText: string) => {
    const aiMessageId = crypto.randomUUID();
    
    // 1. Initialise the message container as empty streaming state
    ...
    // Simulate words streaming at 45ms interval
    const timer = setInterval(() => {
      if (index >= words.length) {
        clearInterval(timer);
        // Turn off streaming cursor flag
        setMessages((prev) => {
          const list = prev[activeThread] ?? [];
          return {
            ...prev,
            [activeThread]: list.map((m) => m.id === aiMessageId ? { ...m, isStreaming: false } : m)
          };
        });
        return;
      }
      ...
      setMessages((prev) => {
        ...
        return {
          ...prev,
          [activeThread]: list.map((m) => m.id === aiMessageId ? { ...m, content: m.content + nextWord } : m)
        };
      });
      index++;
    }, 45);
  ...
  const send = () => {
    ...
    // Clear any active voice synthesis
    if (typeof window !== "undefined") {
      window.speechSynthesis.cancel();
      setPlayingVoiceId(null);
    }
    ...
    // Find the last assistant message from current scope
    ...
    setTimeout(() => {
      setThinking(false);
      const responseText = getAIResponse(text, companies, countries, news, lastAssistantContent);
      streamAIResponse(responseText);
    }, 1200);
  };
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-muted/5">
            ...
            {thinking && (
              <div className="flex gap-3">
                ...
                <div className="bg-card border border-border rounded-lg px-4 py-3 flex items-center gap-2 text-sm text-muted-foreground shadow-sm">
          ...
          <div className="p-4 border-t border-border bg-card/30">
            ...
            <div className="flex gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className={cn(
                    ...
                    isListening && "border-red-500/50 bg-red-50/5 ring-1 ring-red-500/30 placeholder:text-red-400"
                  )}
                />
                
                {/* Voice Dictation (STT) Trigger Button */}
                <button
                  type="button"
                  ...
                  className={cn(
                    ...
                    isListening && "text-red-500 hover:text-red-600 bg-red-100/10 animate-pulse"
                  )}
              ...
              <Button onClick={send} disabled={!input.trim() || thinking || isListening} size="sm" className="h-10 px-4">
                <Send className="h-4 w-4" />
              </Button>
            </div>
            <div className="mt-2 text-[10px] font-mono text-muted-foreground text-center">
              AI responses are estimates and intelligence summaries · Connected to verified company databases
            </div>
          </div>
        </div>
      </div>
