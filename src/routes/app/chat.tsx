...
function getAIResponse(
  ...
  if (lastAssistantContent) {
    ...
    if (
      ...
      if (
        ...
        let comparison =
          "Here is a detailed comparison of the top matches:\n\n";
        top3.forEach((c, idx) => {
    ...
    if (
      lastLower.includes("email template for arvind mills?") ||
      lastLower.includes("email template for")
    ) {
      if (
        lower === "yes" ||
        lower === "yes please" ||
    ...
    if (
      ...
      if (
        ...
        lower.includes("want to check")
      ) {
        const topCompany = companies[0] || { name: "Arvind Mills Ltd" };
        const secondCompany = companies[1] || { name: "Vardhman Textiles" };
        return (
          `### EU Green Deal Compliance Audit - Sourcing Report\n\n` +
  ...
  for (const comp of companies) {
    ...
    if (
      lower.includes(compNameLower) ||
      (comp.slug && lower.includes(comp.slug.replace("-", " ")))
    ) {
      return (
        `Here is the current intelligence profile for **${comp.name}**:\n\n` +
  ...
  if (
    lower.includes("highest trust") ||
    lower.includes("best trust") ||
    lower.includes("top trust") ||
    lower.includes("most trusted")
  ) {
    ...
    if (top) {
      return (
        `The supplier with the highest AI Trust Score in our database is **${top.name}** with a score of **${top.ai_trust_score}/100** (Location: ${top.city ?? "Primary Hub"}, ${getCountryName(top.country_code)}).\n\nOther high trust suppliers include:\n` +
        sorted
          .slice(1, 4)
          .map((c) => `- **${c.name}**: Trust ${c.ai_trust_score}/100`)
          .join("\n") +
  ...
  if (
    ...
    lower.includes("total companies") ||
    lower.includes("how many suppliers")
  ) {
    ...
    return (
      `We are currently tracking **${total} companies** in the GMIntel system:\n` +
  ...
  if (
    ...
    const gotsCertified = companies.filter(
      (c) =>
    ...
    return (
      ...
      gotsCertified
        ...
        .map(
          (c) =>
            `- **${c.name}** (${getCountryName(c.country_code)}) — Trust: ${c.ai_trust_score}/100 | Specializes in certified organic fiber.`,
        )
        .join("\n") +
  ...
  if (lower.includes("india") || lower.includes("indian")) {
    ...
    if (indians.length > 0) {
      return (
        `We track **${indians.length} suppliers in India**:\n\n` +
        indians
          .slice(0, 5)
          .map(
            (c) =>
              `- **${c.name}** (Trust: ${c.ai_trust_score}/100 | Risk: ${c.ai_risk_level})`,
          )
          .join("\n") +
  ...
  if (lower.includes("bangladesh") || lower.includes("bangladeshi")) {
    ...
    if (bd.length > 0) {
      return (
        `We track **${bd.length} suppliers in Bangladesh**:\n\n` +
        bd
          .slice(0, 5)
          .map(
            (c) =>
  ...
  if (lower.includes("turkey") || lower.includes("turkish")) {
    ...
    if (tr.length > 0) {
      return (
        `We track **${tr.length} suppliers in Turkey**:\n\n` +
        tr
          .slice(0, 5)
          .map(
            (c) =>
...
function MessageBubble({
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      >
        <div
          className={`rounded-lg px-4 py-3 text-sm leading-relaxed relative group ${
            isUser
...
function Page() {
  const [threads, setThreads] = useState([
    ...
    },
    {
      ...
      ts: new Date(Date.now() - 3600000 * 5),
    },
  ]);
  ...
  const [messages, setMessages] = useState<Record<string, Message[]>>({
    "1": [
      ...
      },
      {
        ...
        role: "user",
        content:
          "Find me cotton manufacturers in India with GOTS certification",
        ts: new Date(Date.now() - 86400000 + 60000),
      },
      {
        id: "a3",
        role: "assistant",
        content: AI_RESPONSES.supplier,
        ts: new Date(Date.now() - 86400000 + 90000),
      },
    ],
    "2": [
      {
    ...
    const timer = setInterval(() => {
      ...
      setMessages((prev) => {
        ...
        return {
          ...prev,
          [activeThread]: list.map((m) =>
            m.id === aiMessageId ? { ...m, content: m.content + nextWord } : m,
          ),
        };
      });
      index++;
  ...
  const send = () => {
    ...
    setThreads((prev) =>
      prev.map((t) =>
        t.id === activeThread && t.title.startsWith("New")
          ? { ...t, title: text.slice(0, 40) }
          : t,
      ),
    );
    ...
    setTimeout(() => {
      setThinking(false);
      const responseText = getAIResponse(
        text,
        companies,
        countries,
        news,
  ...
  const newThread = () => {
    ...
    setThreads((prev) => [
      { id, title: "New conversation", ts: new Date() },
      ...prev,
    ]);
    setMessages((prev) => ({
      ...prev,
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="p-4 border-t border-border bg-card/30">
            ...
            <div className="mt-2 text-[10px] font-mono text-muted-foreground text-center">
              AI responses are estimates and intelligence summaries · Connected
              to verified company databases
            </div>
          </div>
        </div>
      </div>
