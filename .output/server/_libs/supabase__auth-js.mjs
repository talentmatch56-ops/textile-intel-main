...
function stringToUTF8(str, emit) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (codepoint > 55295 && codepoint <= 56319) {
      ...
      codepoint =
        (((str.charCodeAt(i + 1) - 56320) & 65535) | highSurrogate) + 65536;
      i += 1;
    }
...
async function _request(fetcher, method, url, options) {
  ...
  const data = await _handleRequest(
    ...
    method,
    url +
      (Object.keys(qs).length ? "?" + new URLSearchParams(qs).toString() : ""),
    {
      headers,
      noResolveJson:
...
function _sessionResponse(data) {
  ...
  const user =
    (_a = data.user) !== null && _a !== void 0
      ? _a
      : typeof (data === null || data === void 0 ? void 0 : data.id) ===
          "string"
        ? data
        : null;
  return {
...
function identifyRegistrationError({ error, options }) {
  ...
  if (error.name === "AbortError") {
    ...
    if (
      publicKey.pubKeyCredParams.filter((param) => param.type === "public-key")
        .length === 0
    )
      return new WebAuthnError({
          ...
          .then((factor) =>
            factor
              ? this.client.mfa.unenroll({
                  factorId:
                    factor === null || factor === void 0 ? void 0 : factor.id,
                })
              : void 0,
          );
        return {
...
var GoTrueClient = class GoTrueClient {
  ...
  async signInWithEthereum(credentials) {
    ...
    if ("message" in credentials) {
      ...
      if (!chainId)
        chainId = fromHex(
          await resolvedWallet.request({ method: "eth_chainId" }),
        );
    ...
    try {
      ...
      if (error) {
        if (!!(
          ...
          if (
            stillStored &&
            stillStored.refresh_token === currentSession.refresh_token
          )
            return this._returnResult({
              data: { session: currentSession },
              error: null,
      ...
      if (callbackUrlType === "pkce") {
        ...
        return {
          data: {
            session: data.session,
            redirectType:
              (_a = data.redirectType) !== null && _a !== void 0 ? _a : null,
          },
          error: null,
        };
      }
        ...
        if (this.autoRefreshToken && currentSession.refresh_token) {
          ...
          if (error)
            if (isAuthRefreshDiscardedError(error))
              this._debug(
                debugName,
                "refresh discarded by commit guard",
                error,
              );
  ...
  async _callRefreshToken(refreshToken) {
    ...
    try {
      ...
      if (
        storedAtStart !== null &&
        (storedAfter === null ||
          storedAfter.refresh_token !== storedAtStart.refresh_token)
      ) {
        this._debug(
          debugName,
      ...
      if (isAuthError(error)) {
        ...
        if (!isAuthRetryableFetchError(error)) {
          ...
          if (!!(
            (storedNow === null || storedNow === void 0
              ? void 0
              : storedNow.expires_at) && storedNow.expires_at * 1e3 > Date.now()
          ))
            this._debug(
              debugName,
        ...
        (_a = this.refreshingDeferred) === null ||
          _a === void 0 ||
          _a.resolve(result);
        return result;
      }
      (_b = this.refreshingDeferred) === null ||
        _b === void 0 ||
        _b.reject(error);
      throw error;
    } finally {
  ...
  async _challengeAndVerify(params) {
    const { data: challengeData, error: challengeError } =
      await this._challenge({ factorId: params.factorId });
    if (challengeError)
      return this._returnResult({
        data: null,
        error: challengeError,
