...
function usePointerDownOutside(onPointerDownOutside, args) {
  ...
  const interceptedOutsideInteractionEventsRef = import_react.useRef(
    /* @__PURE__ */ new Map(),
  );
  ...
  import_react.useEffect(() => {
    ...
    function isOutsideInteractionIntercepted() {
      return Array.from(
        interceptedOutsideInteractionEventsRef.current.values(),
      ).some(Boolean);
    }
    ...
    const handlePointerDown = (event) => {
      if (event.target && !isPointerInsideReactTreeRef.current) {
        let handleAndDispatchPointerDownOutsideEvent2 = function () {
          ...
          const wasOutsideInteractionIntercepted =
            isOutsideInteractionIntercepted();
        ...
        isDeferredPointerDownOutsideRef.current =
          deferPointerDownOutside && event.button === 0;
        interceptedOutsideInteractionEventsRef.current.clear();
