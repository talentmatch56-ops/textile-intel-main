...
import { Building2, Factory, Globe2, Layers, Sparkles, TrendingUp, Loader2, Download, X, AlertCircle, RefreshCw, Star, ArrowRight } from "lucide-react";
...
import { MOCK_COMPANY_NEWS, PENDING_NEWS_SIMULATION } from "@/utils/mock-company-news";
...
function Dashboard() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { companies: companies.data ?? [], countries: countries.data ?? [], news: news.data ?? [], prices: prices.data ?? [] };
    },
  });
  ...
  const countryMap = useMemo(() => Object.fromEntries(countries.map((c) => [c.code, c.name])), [countries]);
  ...
  const combinedCompanyNews = useMemo(() => {
    ...
    return [...syncedNews, ...MOCK_COMPANY_NEWS].sort((a, b) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime());
  }, [syncedNewsCount]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <StatCard label="Sync Engine" value={lastSyncTime === "Never" ? "Ready" : lastSyncTime} delta={{ value: syncing ? "polling" : "sync active", positive: true }} icon={<RefreshCw className="h-4 w-4" />} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-lg border border-border bg-card overflow-hidden">
          <div className="p-4 border-b border-border flex items-center justify-between">
            <div>
              <div className="text-[10px] font-mono uppercase tracking-widest text-primary">Sourcing Watchlist</div>
              <div className="font-display font-semibold">Tracked supplier intelligence</div>
            </div>
            <span className="text-[10px] font-mono text-muted-foreground">{watchlistedData.length} monitored</span>
          </div>
          ...
          <div className="divide-y divide-border">
            {watchlistedData.length === 0 ? (
              <div className="p-8 text-center text-sm text-muted-foreground">
                ...
                <p className="text-xs text-muted-foreground/75 mt-1">Start tracking suppliers on the Companies tab to see their latest announcements here.</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="space-y-1.5 flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-foreground hover:text-primary transition-colors text-sm truncate">{c.name}</span>
                        <Badge variant="outline" className="text-[9px] font-mono tracking-wide py-0 px-1 border-0 bg-primary/10 text-primary">Trust {c.ai_trust_score}</Badge>
                        <RiskBadge level={c.ai_risk_level} />
                      </div>
                      
                      {latestCompanyNews ? (
                        <div className="space-y-0.5 pl-3 border-l border-border/80">
                          <div className="text-[10px] font-semibold text-foreground/80 leading-normal truncate">{latestCompanyNews.title}</div>
                          <p className="text-xs text-muted-foreground line-clamp-1 leading-normal">{latestCompanyNews.summary}</p>
                        </div>
                      ) : (
                        <p className="text-xs text-muted-foreground/75 italic pl-3">No updates reported for this supplier.</p>
                      )}
                    </div>
        ...
        <div className="rounded-lg border border-border bg-card overflow-hidden">
          <div className="p-4 border-b border-border">
            <div className="text-[10px] font-mono uppercase tracking-widest text-primary">Announcements Feed</div>
            <div className="font-display font-semibold font-sans">Company updates & sentiment</div>
          </div>
            ...
            {combinedCompanyNews.slice(0, 6).map((n) => (
              <div 
                ...
                onClick={() => {
                  ...
                  if (comp) { setSelectedCompany(comp); setDrawerOpen(true); }
                }}
                className="p-4 hover:bg-muted/40 transition-colors cursor-pointer space-y-1.5"
              >
                <div className="flex items-center justify-between text-[9px] font-mono">
                  <span className="text-primary font-semibold hover:underline">{n.company_name}</span>
                  <Badge 
                    variant="outline" 
                    className={cn(
                      "text-[8px] font-mono px-1 py-0 h-4 border-0 uppercase",
                      n.sentiment === 'positive' && "bg-success/15 text-success",
                      n.sentiment === 'negative' && "bg-destructive/15 text-destructive",
                      n.sentiment === 'neutral' && "bg-muted/80 text-muted-foreground"
                    )}
                  >
                    {n.sentiment}
                  </Badge>
                </div>
                <div className="text-xs font-semibold text-foreground leading-snug line-clamp-2">{n.title}</div>
                <div className="text-[10px] text-muted-foreground flex justify-between font-mono">
                  <span>Source: {n.source}</span>
                  <span>{new Date(n.published_at).toLocaleDateString("en-US", { timeZone: "UTC", month: "short", day: "numeric" })}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {selectedCompany && (
            <div className="space-y-6">
              <SheetHeader className="text-left space-y-3">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <SheetTitle className="font-display text-xl font-bold leading-tight text-foreground">{selectedCompany.name}</SheetTitle>
                      <SheetDescription className="text-xs text-muted-foreground font-mono mt-0.5">
                        {selectedCompany.city ? `${selectedCompany.city}, ` : ""}{countryMap[selectedCompany.country_code] ?? selectedCompany.country_code}
                      </SheetDescription>
                    </div>
                  </div>
                  <Button 
                  ...
                  >
                    <Star className={cn("h-3.5 w-3.5", watchlist.includes(selectedCompany.slug) ? "fill-primary text-primary" : "")} />
                    {watchlist.includes(selectedCompany.slug) ? "Watchlisted" : "Watchlist"}
                  </Button>
                </div>

                <div className="flex flex-wrap gap-2 pt-1 border-b border-border pb-3">
                  <Badge variant="outline" className="text-[10px] font-mono capitalize">{selectedCompany.business_type ?? "Supplier"}</Badge>
                  <Badge variant="outline" className="text-[10px] font-mono">Trust: {selectedCompany.ai_trust_score}/100</Badge>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs bg-muted/30 p-3 rounded-lg border border-border">
                <div>
                  <span className="text-muted-foreground block font-mono text-[9px] uppercase tracking-wide">Capacity</span>
                  <span className="font-medium text-foreground">{selectedCompany.monthly_capacity ? `${selectedCompany.monthly_capacity.toLocaleString()} m/mo` : "—"}</span>
                </div>
                <div>
                  <span className="text-muted-foreground block font-mono text-[9px] uppercase tracking-wide">Min Order Qty</span>
                  <span className="font-medium text-foreground">{selectedCompany.moq ? `${selectedCompany.moq.toLocaleString()} units` : "—"}</span>
                </div>
                <div>
                  <span className="text-muted-foreground block font-mono text-[9px] uppercase tracking-wide">Lead Time</span>
                  <span className="font-medium text-foreground">{selectedCompany.lead_time_days ? `${selectedCompany.lead_time_days} days` : "—"}</span>
                </div>
                <div>
                  <span className="text-muted-foreground block font-mono text-[9px] uppercase tracking-wide">Employees</span>
                  <span className="font-medium text-foreground">{selectedCompany.employees_range ?? "—"}</span>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-primary/30 bg-gradient-to-br from-primary/[0.06] to-transparent p-4">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed font-sans">{aiDrawerSummary}</p>
              </div>

              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-border pb-2">
                  <h4 className="font-display font-semibold text-sm text-foreground">Company Intelligence Feed</h4>
                  <span className="text-[10px] font-mono text-muted-foreground">{activeCompanyNews.length} announcements</span>
                </div>
                ...
                <div className="space-y-3">
                  {activeCompanyNews.length === 0 ? (
                    ...
                    activeCompanyNews.map((news) => (
                      <div key={news.id} className="p-3 border border-border rounded-lg bg-card/40 hover:border-primary/30 transition-colors">
                        <div className="flex items-center justify-between gap-2 mb-1.5">
                          ...
                          <span className="text-[9px] font-mono text-muted-foreground/80">
                            {new Date(news.published_at).toLocaleDateString("en-US", { timeZone: "UTC", month: "short", day: "numeric" })}
                          </span>
                        </div>
                        <h5 className="text-xs font-semibold text-foreground leading-snug mb-1">{news.title}</h5>
                        <p className="text-[11px] text-muted-foreground leading-relaxed mb-2.5">{news.summary}</p>
                        
                        <div className="flex items-center justify-between pt-1.5 border-t border-border/50 text-[10px] font-mono">
                          <span className="text-muted-foreground/75">Source: {news.source}</span>
                          <div className="flex items-center gap-1.5">
                            <span className="text-muted-foreground/65">Sentiment:</span>
                            <Badge 
                              variant="outline" 
                              className={cn(
                                "text-[9px] px-1 py-0 border-0 h-4 uppercase",
                                news.sentiment === 'positive' && "bg-success/15 text-success",
                                news.sentiment === 'negative' && "bg-destructive/15 text-destructive",
                                news.sentiment === 'neutral' && "bg-muted/80 text-muted-foreground"
                              )}
                            >
                              {news.sentiment}
                            </Badge>
